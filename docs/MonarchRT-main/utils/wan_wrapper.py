...
class WanVAEWrapper_(torch.nn.Module):
    def __init__(self):
        super().__init__()
        mean = [
...
class WanVAEDecoder:
    ...
    def __call__(self, latent, use_cache=False):
        # from [batch_size, num_frames, num_channels, height, width]
        # to [batch_size, num_channels, num_frames, height, width]
        latent = latent.permute(0, 2, 1, 3, 4).contiguous(memory_format=torch.channels_last_3d)
        latent = latent.permute(0, 2, 1, 3, 4)
        return self.vae.decode_to_pixel(latent, use_cache)
...
class WanVAEEncoder:
    ...
    def __call__(self, pixel: torch.Tensor):
        return self.vae.encode_to_latent(pixel)

class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.vae = WanVAEWrapper_()
        ...
        self.encoder = WanVAEEncoder(self.vae)
        self.decoder = WanVAEDecoder(self.vae)

    def to(self, *args, **kwargs):
        super().to(*args, **kwargs)
        # after device/dtype move, convert conv3d weights
        self.vae = torch.nn.utils.convert_conv3d_weight_memory_format(self.vae, torch.channels_last_3d)
        self.vae = torch.nn.utils.convert_conv2d_weight_memory_format(self.vae, torch.channels_last)
        self.encoder = WanVAEEncoder(self.vae)
        self.decoder = WanVAEDecoder(self.vae)
