...
class BaseModel(nn.Module):
    ...
    def _initialize_models(self, args, device):
        ...
        self.generator = WanDiffusionWrapper(**getattr(args, "model_kwargs", {}), is_causal=getattr(args, "causal", False), image_or_video_shape=args.image_or_video_shape)
        self.generator.model.requires_grad_(True)

        self.real_score = WanDiffusionWrapper(model_name=self.real_model_name, is_causal=False, image_or_video_shape=args.image_or_video_shape)
        self.real_score.model.requires_grad_(False)

        self.fake_score = WanDiffusionWrapper(model_name=self.fake_model_name, is_causal=False, image_or_video_shape=args.image_or_video_shape)
    ...
    def _run_generator(
        ...
        if pred_image_or_video.shape[1] > 21:
            with torch.no_grad():
                ...
                # Deccode to video
                pixels = self.vae.decoder(latent_to_decode)
                frame = pixels[:, -1:, ...].to(self.dtype)
                frame = rearrange(frame, "b t c h w -> b c t h w")
                # Encode frame to get image latent
                image_latent = self.vae.encoder(frame).to(self.dtype)
