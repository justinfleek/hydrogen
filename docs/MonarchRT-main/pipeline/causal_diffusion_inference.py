...
class CausalDiffusionInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        self.generator = WanDiffusionWrapper(
            **getattr(args, "model_kwargs", {}), is_causal=True, image_or_video_shape=args.image_or_video_shape) if generator is None else generator
        ...
        for i in range(3):
            video_shape[i] //= self.generator.model.patch_size[i]
        num_frames, frame_height, frame_width = video_shape
        self.total_seq_len = num_frames * frame_height * frame_width
    ...
    def inference(
        ...
        if self.kv_cache_pos is None:
            ...
            for block_index in range(len(self.kv_cache_pos)):
                self.kv_cache_pos[block_index]["global_end_index"] = 0
                self.kv_cache_pos[block_index]["local_end_index"] = 0
                self.kv_cache_neg[block_index]["global_end_index"] = 0
                self.kv_cache_neg[block_index]["local_end_index"] = 0
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        if self.local_attn_size != -1:
            ...
            kv_cache_size = self.total_seq_len
