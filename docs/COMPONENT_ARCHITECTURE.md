...
# Hydrogen Component Architecture

**Version:** 1.0.0
**Status:** Canonical
**Author:** Claude (Opus 4.5) + jpyxal

This document defines how Hydrogen components consume Schema atoms to produce
...
2. **Framework Lock-in** — Components are tied to React, Vue, Svelte, etc.
...
## Schema-Native Components

Hydrogen components are **Schema-native** — they accept Schema atoms directly
...
2. **Framework independence** — Components produce `Element msg`, which can
...
Every Hydrogen component has three layers:
...
Every component defines sensible defaults that produce a usable component
even with zero configuration:

```purescript
defaultProps :: forall msg. ComponentProps msg
defaultProps =
  { content: ""
  , onClick: Nothing
...
type ComponentProp msg = ComponentProps msg -> ComponentProps msg

-- Modifier functions
backgroundColor :: forall msg. Color.RGB -> ComponentProp msg
backgroundColor c props = props { backgroundColor = Just c }

borderRadius :: forall msg. Geometry.Corners -> ComponentProp msg
borderRadius r props = props { borderRadius = Just r }

glow :: forall msg. Color.Glow -> ComponentProp msg
...
Components don't hardcode values — they receive atoms from a BrandSchema:
...
```purescript
-- Components can define state-based styling
...
component
  [ ...normal props...
  , extraAttributes [ E.dataAttr "testid" "my-component" ]
  ]
```
...
```purescript
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
--                                         // hydrogen // element // componentname
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
-- | ## Schema Atoms
-- |
-- | This component accepts atoms from the following pillars:
-- |
...
-- |
-- | ```purescript
-- | import Hydrogen.Element.Component.ComponentName as Component
-- | import Hydrogen.Schema.Color as Color
-- | import Hydrogen.Schema.Geometry as Geometry
-- |
-- | -- Minimal usage
-- | Component.component [ Component.content "Hello" ]
-- |
-- | -- With Schema atoms
-- | Component.component
-- |   [ Component.content "Hello"
-- |   , Component.backgroundColor (Color.rgb 59 130 246)
-- |   , Component.borderRadius (Geometry.cornersAll Geometry.lg)
-- |   ]
-- |
-- | -- With brand injection
-- | Component.component
-- |   [ Component.content "Hello"
-- |   , Component.backgroundColor brand.primary
-- |   , Component.borderRadius brand.corners
-- |   , Component.glow brand.accentGlow
-- |   ]
-- | ```

module Hydrogen.Element.Component.ComponentName
  ( -- * Component
    component
...
-- | Set content
content :: forall msg. String -> ComponentProp msg
content c props = props { content = c }

-- | Set click handler
onClick :: forall msg. msg -> ComponentProp msg
onClick handler props = props { onClick = Just handler }

-- | Set disabled state
disabled :: forall msg. Boolean -> ComponentProp msg
...
textColor :: forall msg. Color.RGB -> ComponentProp msg
...
padding :: forall msg. Dimension.Spacing -> ComponentProp msg
padding p props = props { padding = Just p }

-- | Set shadow (Elevation pillar)
shadow :: forall msg. Elevation.Shadow -> ComponentProp msg
shadow s props = props { shadow = Just s }

-- | Set animation (Motion pillar)
animation :: forall msg. Motion.Animation -> ComponentProp msg
animation a props = props { animation = Just a }

-- | Add extra attributes (escape hatch)
extraAttributes :: forall msg. Array (E.Attribute msg) -> ComponentProp msg
...
component :: forall msg. Array (ComponentProp msg) -> E.Element msg
component propMods =
  let
    props = foldl (\p f -> f p) defaultProps propMods
...
buildStyles :: forall msg. ComponentProps msg -> Array (E.Attribute msg)
buildStyles props =
  let
...
clickAttr :: forall msg. ComponentProps msg -> Array (E.Attribute msg)
...
1. **Components are Schema-native** — They accept atoms, not strings.

2. **No hardcoded values** — Defaults are `Nothing` (inherit) or neutral.

3. **Atoms → CSS** — Each pillar has `toCss` functions for conversion.

4. **Brand injection** — Production code passes brand atoms to components.

5. **Showcase previews** — Same components, same atoms, deterministic output.
